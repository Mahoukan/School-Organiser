...
export function getAdjacentClassOccurrences(entries, selection, maximumDays = 365) {
  const currentIndex = entries.findIndex(
    (entry) => entry.date === selection.date &&
      entry.recurringAssignmentId === selection.recurringAssignmentId,
  );
  if (currentIndex < 0) return { previous: null, next: null };
  const currentDate = parseDateOnly(selection.date);
  const withinBound = (entry) =>
    Math.abs(parseDateOnly(entry.date) - currentDate) / 86_400_000 <= maximumDays;
  return {
    previous: entries.slice(0, currentIndex).reverse().find(withinBound) ?? null,
    next: entries.slice(currentIndex + 1).find(withinBound) ?? null,
  };
}
