...
export async function getSchoolData(userId, knownPreferences) {
  const db = getDatabase();
  const [years, preferences] = await Promise.all([
    db.select().from(schema.academicYears).where(and(eq(schema.academicYears.userId, userId), eq(schema.academicYears.active, true))).limit(1),
    knownPreferences ? Promise.resolve(knownPreferences) : getUserPreferences(userId),
  ]);
  const academicYear = years[0];
  if (!academicYear) return { preferences, academicYear: null, terms: [], teachingWeeks: [], classes: [], dayTimetableTemplates: [], dayTimetableTemplateBlocks: [], dayTimetableAssignments: [], timetableBlocks: [], recurringAssignments: [], historicalRecurringAssignments: [], recurringEvents: [], datedEvents: [], lessonOccurrences: [], teacherAbsences: [], classAbsences: [], calendarExceptions: [], lessonMovements: [] };
  ...
  const ownedClassIds = new Set(classRows.map((classItem) => classItem.id));
  ...
  const absenceLinks = absenceIds.length ? (await db.select().from(schema.classAbsenceClasses).where(inArray(schema.classAbsenceClasses.classAbsenceId, absenceIds))).filter((link) => ownedClassIds.has(link.classId)) : [];
...
export async function mutateSchoolData(userId, resource, action, payload) {
  ...
  await db.transaction(async (tx) => {
    if (resource === "classes") {
      ...
      assertOwned(state.timetableBlocks, details.periodId, "Period block");
      ...
      if (payload.kind === "term") {
        ...
        else if (action === "generate") { const term = assertOwned(state.terms, payload.termId, "Term"); const generated = generateMissingWeeks(term, state.teachingWeeks, payload.firstCycleWeek); if (generated.length) await tx.insert(schema.teachingWeeks).values(generated.map((week) => ({ ...week, id: id("teaching-week") }))); }
        else { const values = payload.values; if (values.id) assertOwned(state.teachingWeeks, values.id, "Teaching week"); const term = assertOwned(state.terms, values.termId, "Term"); const errors = validateTeachingWeek(values, term, state.teachingWeeks, values.id); if (Object.keys(errors).length) throw new Error(Object.values(errors)[0]); const row = { termId: values.termId, weekStartDate: values.weekStartDate, cycleWeek: values.cycleWeek, updatedAt: new Date() }; if (values.id) await tx.update(schema.teachingWeeks).set(row).where(eq(schema.teachingWeeks.id, values.id)); else await tx.insert(schema.teachingWeeks).values({ id: id("teaching-week"), ...row }); }
      }
  ...
  return getSchoolData(userId, state.preferences);
}
