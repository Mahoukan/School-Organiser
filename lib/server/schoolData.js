...
import { validateTimetableBlock } from "../periodStructures.js";
...
export const DEVELOPMENT_USER_ID = "development-user";
...
function publicBlock(item) { return { id: item.id, cycleWeek: item.cycleWeek, weekday: item.weekday, name: item.name, startTime: item.startTime, endTime: item.endTime, displayOrder: item.displayOrder, isTeaching: item.isTeaching }; }
...
export async function getSchoolData() {
  ...
  const years = await db.select().from(schema.academicYears).where(and(eq(schema.academicYears.userId, DEVELOPMENT_USER_ID), eq(schema.academicYears.active, true))).limit(1);
  const academicYear = years[0];
  if (!academicYear) return { academicYear: null, terms: [], teachingWeeks: [], classes: [], timetableBlocks: [], recurringAssignments: [], historicalRecurringAssignments: [], recurringEvents: [], lessonOccurrences: [], teacherAbsences: [], classAbsences: [], calendarExceptions: [], lessonMovements: [] };
  const [termRows, classRows, blockRows, recurringRows, lessonRows, teacherRows, absenceRows, absenceLinks, exceptionRows, movementRows] = await Promise.all([
    db.select().from(schema.terms).where(eq(schema.terms.academicYearId, academicYear.id)),
    db.select().from(schema.classes).where(and(eq(schema.classes.userId, DEVELOPMENT_USER_ID), eq(schema.classes.academicYearId, academicYear.id))),
    db.select().from(schema.periodBlocks).where(and(eq(schema.periodBlocks.userId, DEVELOPMENT_USER_ID), eq(schema.periodBlocks.academicYearId, academicYear.id))),
    db.select().from(schema.recurringTimetableItems).where(and(eq(schema.recurringTimetableItems.userId, DEVELOPMENT_USER_ID), eq(schema.recurringTimetableItems.academicYearId, academicYear.id))),
    db.select().from(schema.lessonOccurrences).where(eq(schema.lessonOccurrences.userId, DEVELOPMENT_USER_ID)),
    db.select().from(schema.teacherAbsences).where(eq(schema.teacherAbsences.userId, DEVELOPMENT_USER_ID)),
    db.select().from(schema.classAbsences).where(eq(schema.classAbsences.userId, DEVELOPMENT_USER_ID)),
    db.select().from(schema.classAbsenceClasses),
    db.select().from(schema.calendarExceptions).where(eq(schema.calendarExceptions.userId, DEVELOPMENT_USER_ID)),
    db.select().from(schema.lessonMovements).where(eq(schema.lessonMovements.userId, DEVELOPMENT_USER_ID)),
  ]);
  ...
  return {
    ...
    timetableBlocks: blockRows.map(publicBlock),
...
async function retireRecurringItem(tx, item, state) {
  const today = nzDate();
  if (item.effectiveFromDate >= today) {
    ...
    await tx.delete(schema.recurringTimetableItems).where(and(eq(schema.recurringTimetableItems.id, item.id), eq(schema.recurringTimetableItems.userId, DEVELOPMENT_USER_ID)));
  } else {
    await tx.update(schema.recurringTimetableItems).set({ effectiveToDate: nzDate(-1), updatedAt: new Date() }).where(and(eq(schema.recurringTimetableItems.id, item.id), eq(schema.recurringTimetableItems.userId, DEVELOPMENT_USER_ID)));
  }
}

export async function mutateSchoolData(resource, action, payload) {
  const db = getDatabase();
  const state = await getSchoolData();
  if (!state.academicYear) throw new Error("Run the database seed before making changes.");
  ...
  await db.transaction(async (tx) => {
    if (resource === "classes") {
      if (action === "save") {
        ...
        if (payload.id) { assertOwned(state.classes, payload.id, "Class"); await tx.update(schema.classes).set(row).where(and(eq(schema.classes.id, payload.id), eq(schema.classes.userId, DEVELOPMENT_USER_ID))); }
        else await tx.insert(schema.classes).values({ id: id("class"), userId: DEVELOPMENT_USER_ID, academicYearId: yearId, ...row });
        ...
        await tx.update(schema.classes).set({ archived: action === "archive", updatedAt: new Date() }).where(and(eq(schema.classes.id, payload.id), eq(schema.classes.userId, DEVELOPMENT_USER_ID)));
      }
      ...
      if (action === "save-class") {
        ...
        const today = nzDate();
        if (existing) await retireRecurringItem(tx, existing, state);
        await tx.insert(schema.recurringTimetableItems).values({ id: id("assignment"), userId: DEVELOPMENT_USER_ID, academicYearId: yearId, kind: "class", classId: payload.classId, cycleWeek: payload.cycleWeek, weekday: payload.weekday, periodId: payload.periodId, effectiveFromDate: today });
        ...
        await retireRecurringItem(tx, existing, state);
        ...
        if (values.id) { assertOwned(state.recurringEvents, values.id, "Recurring item"); await tx.update(schema.recurringTimetableItems).set(row).where(and(eq(schema.recurringTimetableItems.id, values.id), eq(schema.recurringTimetableItems.userId, DEVELOPMENT_USER_ID))); }
        else await tx.insert(schema.recurringTimetableItems).values({ id: id("event"), userId: DEVELOPMENT_USER_ID, academicYearId: yearId, kind: "event", cycleWeek: values.cycleWeek, weekday: values.weekday, periodId: values.periodId, effectiveFromDate: nzDate(), ...row });
      } else if (action === "remove-event") {
        assertOwned(state.recurringEvents, payload.id, "Recurring item");
        await tx.delete(schema.recurringTimetableItems).where(and(eq(schema.recurringTimetableItems.id, payload.id), eq(schema.recurringTimetableItems.userId, DEVELOPMENT_USER_ID), eq(schema.recurringTimetableItems.kind, "event")));
      }
      ...
      if (!hasLessonContent(content)) await tx.delete(schema.lessonOccurrences).where(and(eq(schema.lessonOccurrences.userId, DEVELOPMENT_USER_ID), eq(schema.lessonOccurrences.date, details.date), eq(schema.lessonOccurrences.recurringAssignmentId, details.recurringAssignmentId)));
      else await tx.insert(schema.lessonOccurrences).values({ id: occurrenceId, userId: DEVELOPMENT_USER_ID, date: details.date, recurringAssignmentId: details.recurringAssignmentId, classId: details.classId, periodId: details.periodId, ...content }).onConflictDoUpdate({ target: [schema.lessonOccurrences.userId, schema.lessonOccurrences.date, schema.lessonOccurrences.recurringAssignmentId], set: { periodId: details.periodId, ...content, updatedAt: new Date() } });
      ...
      if (action === "remove") { assertOwned(list, payload.id, "Overlay"); await tx.delete(table).where(and(eq(table.id, payload.id), eq(table.userId, DEVELOPMENT_USER_ID))); }
      else {
        const values = payload.values;
        ...
        await tx.insert(table).values({ id: recordId, userId: DEVELOPMENT_USER_ID, ...row }).onConflictDoUpdate({ target: table.id, set: { ...row, updatedAt: new Date() } });
    ...
    } else if (resource === "period-blocks") {
      if (action === "remove") {
        assertOwned(state.timetableBlocks, payload.id, "Period block");
        if (state.recurringAssignments.some((item) => item.periodId === payload.id) || state.recurringEvents.some((item) => item.periodId === payload.id) || state.lessonMovements.some((item) => item.destinationPeriodId === payload.id)) throw new Error("Remove assignments, recurring items and movements from this block first.");
        await tx.delete(schema.periodBlocks).where(and(eq(schema.periodBlocks.id, payload.id), eq(schema.periodBlocks.userId, DEVELOPMENT_USER_ID)));
      } else if (action === "move") {
        const block = assertOwned(state.timetableBlocks, payload.id, "Period block"); const sibling = state.timetableBlocks.filter((item) => item.cycleWeek === block.cycleWeek && item.weekday === block.weekday).sort((a, b) => a.displayOrder - b.displayOrder); const index = sibling.findIndex((item) => item.id === block.id); const swap = sibling[index + payload.direction]; if (swap) { await tx.update(schema.periodBlocks).set({ displayOrder: swap.displayOrder }).where(eq(schema.periodBlocks.id, block.id)); await tx.update(schema.periodBlocks).set({ displayOrder: block.displayOrder }).where(eq(schema.periodBlocks.id, swap.id)); }
      } else { const values = payload.values; const errors = validateTimetableBlock(values, state.timetableBlocks, values.id); if (Object.keys(errors).length) throw new Error(Object.values(errors)[0]); const existing = values.id ? assertOwned(state.timetableBlocks, values.id, "Period block") : null; if (existing?.isTeaching && !values.isTeaching && (state.recurringAssignments.some((item) => item.periodId === values.id) || state.lessonMovements.some((item) => item.destinationPeriodId === values.id))) throw new Error("Remove class assignments and movements before making this block non-teaching."); const row = { userId: DEVELOPMENT_USER_ID, academicYearId: yearId, cycleWeek: values.cycleWeek, weekday: values.weekday, name: values.name.trim(), startTime: values.startTime, endTime: values.endTime, displayOrder: existing?.displayOrder ?? Math.max(0, ...state.timetableBlocks.filter((item) => item.cycleWeek === values.cycleWeek && item.weekday === values.weekday).map((item) => item.displayOrder)) + 1, isTeaching: Boolean(values.isTeaching) }; await tx.insert(schema.periodBlocks).values({ id: values.id ?? id("block"), ...row }).onConflictDoUpdate({ target: schema.periodBlocks.id, set: { ...row, updatedAt: new Date() } }); }
      ...
      if (action === "remove") {
        ...
        await tx.delete(schema.lessonMovements).where(and(eq(schema.lessonMovements.userId, DEVELOPMENT_USER_ID), eq(schema.lessonMovements.date, payload.date), eq(schema.lessonMovements.recurringAssignmentId, payload.recurringAssignmentId)));
      }
      else { const validation = validateLessonMovement({ ...payload.values, recurringAssignments: state.recurringAssignments, timetableBlocks: state.timetableBlocks, lessonMovements: state.lessonMovements, recurringEvents: state.recurringEvents, teachingWeeks: state.teachingWeeks }); if (!validation.ok) throw new Error(validation.message); const values = payload.values; await tx.insert(schema.lessonMovements).values({ id: id("movement"), userId: DEVELOPMENT_USER_ID, date: values.date, recurringAssignmentId: values.recurringAssignmentId, destinationPeriodId: values.destinationPeriodId }).onConflictDoUpdate({ target: [schema.lessonMovements.userId, schema.lessonMovements.date, schema.lessonMovements.recurringAssignmentId], set: { destinationPeriodId: values.destinationPeriodId, updatedAt: new Date() } }); }
