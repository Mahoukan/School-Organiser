...
export async function getSchoolData(userId) {
  ...
  const [termRows, classRows, blockRows, recurringRows, lessonRows, teacherRows, absenceRows, exceptionRows, movementRows] = await Promise.all([
  ...
  await db.transaction(async (tx) => {
    if (resource === "classes") {
      ...
      if (action === "remove") {
        ...
        await tx.delete(schema.periodBlocks).where(and(eq(schema.periodBlocks.id, payload.id), eq(schema.periodBlocks.userId, userId)));
      } else if (action === "move") {
