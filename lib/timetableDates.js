...
const WEEK_A_REFERENCE = new Date(2026, 6, 20, 12);
...
export function getWeekType(date) {
  // Prototype-only alternating cycle. Stage 9 will replace this with configured
  // teaching-week records so holidays and manual A/B corrections are supported.
  ...
  const referenceUtc = Date.UTC(
    WEEK_A_REFERENCE.getFullYear(),
    WEEK_A_REFERENCE.getMonth(),
    WEEK_A_REFERENCE.getDate(),
  );
  const weeksSinceReference = Math.round(
    (mondayUtc - referenceUtc) / (7 * MILLISECONDS_PER_DAY),
  );
  const cycleIndex = ((weeksSinceReference % 2) + 2) % 2;

  return cycleIndex === 0 ? "A" : "B";
}

export function getFortnightStart(date) {
  const monday = getMonday(date);
  return getWeekType(monday) === "A" ? monday : addDays(monday, -7);
}
