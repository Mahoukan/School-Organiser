...
export default function DatedEventsManager({ contextualDate = null }) {
  ...
  const list = (events) => <div className={styles.managerList}>{events.map((event) => <div className={styles.managerRow} data-contextual={contextualDate === event.date ? "true" : undefined} key={event.id}><div><span>{formatCalendarDate(event.date, { weekday: "short", day: "numeric", month: "short" })}</span><DatedEventCard event={event} onSelect={open} /></div><div className={styles.rowActions}><Link href={getTimetableUrl({ date: event.date, view: "day" })}>View day</Link><button type="button" className={styles.deleteLink} onClick={(e) => { deleteTrigger.current = e.currentTarget; setMessage(""); setRemoveTarget(event); }}>Delete</button></div></div>)}</div>;
  return <section id="events" className={styles.manager} aria-labelledby="one-off-events-title">
