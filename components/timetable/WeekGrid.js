...
export default function WeekGrid({ monday, compact = false, onOpenLesson }) {
  ...
  return <div className={`${styles.weekDayColumns} ${compact ? styles.compactWeekColumns : ""}`} aria-label={`Week ${teachingWeek.cycleWeek} timetable`}>
    {weekdays.map((weekday, index) => {
      ...
      return <section className={styles.weekDayColumn} key={weekday.key}><header><strong>{weekday.shortLabel}</strong><span>{date.getDate()}</span></header>{blocks.length ? blocks.map((period) => <div className={`${styles.weekColumnBlock} ${!period.isTeaching ? styles.weekColumnBreak : ""}`} key={period.id}><div className={styles.weekBlockMeta}><strong>{period.name}</strong>{!compact && <span>{formatBlockTime(period.startTime)}–{formatBlockTime(period.endTime)}</span>}</div><TimetableCard entry={getTimetableEntry(recurringAssignments, teachingWeek.cycleWeek, weekday.key, period)} detail={compact ? "fortnight" : "week"} date={date} period={period} onOpenLesson={onOpenLesson} /></div>) : <p className={styles.noDayBlocks}>No blocks configured.</p>}</section>;
    })}
  </div>;
}
